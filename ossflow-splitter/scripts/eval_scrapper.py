...
Exit codes:
    ...
    3 - scrape structural checks failed (wrong volumes/chapters/timestamps)
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not run ChapterSplitter.split(); only verify scrape data.",
    )
    p.add_argument(
        "--log-level",
        default="INFO",
...
def _run_search(provider, title: str, author: str, checks: list[Check]) -> Optional[object]:
    from chapter_splitter.scrapper import ScraperError
...
def _verify_scrape(scrape, checks: list[Check]) -> bool:
    """Apply all Tripod Passing structural expectations. Returns True if all OK."""
    all_ok = True

    n_vols = len(scrape.volumes)
    print(f"\nScrape summary: {n_vols} volumes, provider={scrape.provider_id}")
    for v in scrape.volumes:
        print(
...
def _persist_scrape(instructional_dir: Path, scrape) -> None:
    ...
    if meta_file.exists():
        try:
            ...
            data = {}
    # Sidecar JSON key ``oracle`` se preserva por compat con ossflow-api y
    # con .bjj-meta.json ya persistidos en NAS de usuarios.
    data["oracle"] = json.loads(scrape.model_dump_json())
...
def _run_split(instructional_dir: Path, scrape, checks: list[Check]) -> None:
    from chapter_splitter.splitting.chapter_splitter import ChapterSplitter

    splitter = ChapterSplitter(instructional_dir=instructional_dir, scrape_result=scrape)
...
def main() -> int:
    ...
    from chapter_splitter.scrapper import ScraperError, discover, registry
    ...
    try:
        scrape = provider.scrape(top.url)
    except ScraperError as e:
    ...
    scrape_ok = _verify_scrape(scrape, checks)
    if not scrape_ok:
        _print_summary(checks)
        return 3
    ...
    if instructional_dir.exists():
        try:
            _persist_scrape(instructional_dir, scrape)
            checks.append(Check(".bjj-meta.json updated with scrape result", True))
        except Exception as e:
            checks.append(Check(".bjj-meta.json updated with scrape result", False, str(e)))
    else:
        checks.append(
            Check(
                ".bjj-meta.json updated with scrape result",
                False,
    ...
    if not args.dry_run:
        if not instructional_dir.exists():
            ...
            try:
                _run_split(instructional_dir, scrape, checks)
            except Exception as e:
